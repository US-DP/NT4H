package org.springframework.samples.nt4h.turn.hero_attack;
...
public class HeroAttackController {
    ...
    @PostMapping
    public String modifyCardAttributes(Turn turn, HttpSession session) throws NoCurrentPlayerException, WithOutAbilityException {
    ...
    public String attackEnemy(HttpSession session) throws NoCurrentPlayerException, WithOutAbilityException, WithOutEnemyException {
        Player player = getPlayer();
        Game game = getGame();
        Player loggedPlayer = getLoggedPlayer();
        if (loggedPlayer != player)
