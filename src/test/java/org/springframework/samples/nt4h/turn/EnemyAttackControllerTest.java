...
import org.springframework.samples.nt4h.turn.enemy_attack.EnemyAttackController;
...
@ContextConfiguration(classes = {EnemyAttackController.class})
@ExtendWith(SpringExtension.class)
class EnemyAttackControllerTest {
    @MockBean
    ...
    @Test
    void testGetEnemyAttack() {
        when(userService.getLoggedUser()).thenReturn(user);
        ModelMap model = new ModelMap();
        MockHttpSession mockHttpSession = new MockHttpSession();
        assertEquals("turns/attackPhase",
            enemyAttackController.getEnemyAttack(model, mockHttpSession, new MockHttpServletRequest()));
    }

    @Test
    void testNextTurn() throws Exception {
        when(userService.getLoggedUser()).thenReturn(user);
        MockHttpServletRequestBuilder requestBuilder = MockMvcRequestBuilders.get("/enemyAttack/next");
        MockMvcBuilders.standaloneSetup(enemyAttackController)
            .build()
