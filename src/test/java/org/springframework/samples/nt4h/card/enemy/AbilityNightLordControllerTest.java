...
@ContextConfiguration(classes = {AbilityNightLordController.class})
@ExtendWith(SpringExtension.class)
class AbilityNightLordControllerTest {
    @Autowired
    private AbilityNightLordController abilityNightLordController;
    ...
    @Test
    void testGurdrug() throws Exception {
        when(userService.getLoggedUser()).thenReturn(user);
        when(abilityService.getAbilityInGameById(anyInt())).thenReturn(abilityInGame);
        MockHttpServletRequestBuilder requestBuilder = MockMvcRequestBuilders
            .get("/abilities/nightLord/gurdrug/{cardId}", 123);
        MockMvcBuilders.standaloneSetup(abilityNightLordController)
            .build()
            ...
            .andExpect(MockMvcResultMatchers.view().name("redirect:/abilitiesPath Name"))
            .andExpect(MockMvcResultMatchers.redirectedUrl("/abilitiesPath Name"));

    }


    @Test
    void testRoghkiller() throws Exception {
        when(userService.getLoggedUser()).thenReturn(user);
        when(abilityService.getAbilityInGameById(anyInt())).thenReturn(abilityInGame);
        MockHttpServletRequestBuilder requestBuilder = MockMvcRequestBuilders
            .get("/abilities/nightLord/roghkiller/{cardId}", 123);
        MockMvcBuilders.standaloneSetup(abilityNightLordController)
            .build()
            .perform(requestBuilder)
            .andExpect(MockMvcResultMatchers.status().isFound())
    ...
    @Test
    void testShriekknifer() throws Exception {
        when(userService.getLoggedUser()).thenReturn(user);
        when(abilityService.getAbilityInGameById(anyInt())).thenReturn(abilityInGame);
        MockHttpServletRequestBuilder requestBuilder = MockMvcRequestBuilders
            .get("/abilities/nightLord/shriekknifer/{cardId}", 123);
        MockMvcBuilders.standaloneSetup(abilityNightLordController)
            .build()
            .perform(requestBuilder)
            .andExpect(MockMvcResultMatchers.status().isFound())
